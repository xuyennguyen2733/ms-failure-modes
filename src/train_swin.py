...
def main(args):
    ...
    if args.patch_size % 32 != 0:
        raise ValueError(f"SwinUNETR requires patch_size divisible by 32; got {args.patch_size}")
    ...
    for epoch in range(epoch_num):
        print("-" * 10)
        print(f"[Swin] epoch {epoch + 1}/{epoch_num}")
        model.train()
        epoch_loss = 0
        step = 0
        n_iters = 0
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            if metric > best_metric:
                ...
                print("[Swin] saved new best metric model")
            print(f"[Swin] current epoch: {epoch + 1} current mean dice: {metric:.4f}"
    ...
    print(f"[Swin] final-epoch validation mean dice: {final_metric:.4f}")
    if final_metric > best_metric:
        best_metric = final_metric
        best_metric_epoch = epoch_num
        torch.save(model.state_dict(), os.path.join(path_save, "Best_model_finetuning.pth"))
        print(f"[Swin] final model saved as new best ({final_metric:.4f})")
    else:
        print(f"[Swin] final model NOT saved — in-run best {best_metric:.4f} "
